...
// symlinkSharePasswordHash is the bcrypt hash of the password "password"; the
// fork requires every public share to be password-protected.
const symlinkSharePasswordHash = "$2y$10$TFAmdCbyd/mEZDe5fUeZJu.MaJQXRTwdqb/IQV.eTn6dWrF58gCSe"

// withSharePassword sets the credentials the fork requires for any public
// share request (shares must be password-protected) and assigns a unique
// remote address so the per-IP share rate limiter is not tripped across the
// table-driven subtests.
func withSharePassword(r *http.Request) {
	r.Header.Set("X-SHARE-PASSWORD", "password")
	r.RemoteAddr = fmt.Sprintf("10.2.0.%d:12345", testIPCounter.Add(1))
}
...
func symlinkShareStorage(t *testing.T) *storage.Storage {
	...
	if err := st.Share.Save(&share.Link{Hash: "h", UserID: 1, Path: "/shared", PasswordHash: symlinkSharePasswordHash}); err != nil {
