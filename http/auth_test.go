...
//
// Our signup validates the username charset, so the upstream "teamone/x" vector
// is rejected earlier; this uses collapsing dashes ("team--x" -> "team-x"),
// which passes validation yet still collides on the derived scope.
func TestSignupRejectsCollidingNormalizedScope(t *testing.T) {
	...
	if rec := signup("team-x"); rec.Code != http.StatusOK {
	...
	if rec := signup("team--x"); rec.Code != http.StatusConflict {
	...
	owner, err := st.Users.GetByScope("/users/team-x")
	if err != nil {
